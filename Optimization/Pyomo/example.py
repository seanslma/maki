...
# Sets: tech, time
m.tk = pyo.Set(initialize=['gas', 'bio']) #gas, biomass
m.ts = pyo.Set(initialize=[t for t in range(1, 6)], ordered=True)    

# Param: demand[time]
demand = [60, 100, 120, 80, 30]
m.demand = pyo.Param(m.ts, initialize=dict(zip(m.ts, demand)), within=pyo.Any)
